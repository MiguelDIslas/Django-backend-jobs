...
# Change lasdate to lastdate in model
...
def getJob(request, pk):
    ...
    candiates = job.candidatesapplied_set.all().count()
    serializer = JobSerializer(job, many=False)
    return Response({"job": serializer.data, "candidates": candiates})
...
def updateJob(request, pk):
    ...
    if job.user != request.user:
        return Response({"message": "You cannot update this job"},
...
def getTopicStats(request, topic):
    ...
    stats = jobs.aggregate(
        ...
        avg_positions=Round(Avg("positions"), 0),
...
def applyToJob(request, pk):
    ...
    if job.lastDate < timezone.now():
        return Response(
            {"error": "You cannnot apply to this job. Date is Over."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if alreadyApplied:
        return Response(
            {"error": "You have already apply to this job."},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def getCandidatesApplied(request, pk):
    ...
    if job.user != user:
        return Response({'error': 'You cant not access this job'},
                        status=status.HTTP_403_FORBIDDEN)
